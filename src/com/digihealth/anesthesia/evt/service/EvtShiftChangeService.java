...
public class EvtShiftChangeService extends BaseService {
	 ...
	 */
	@Transactional
	public void saveShiftChangeSYZXYY(EvtShiftChange shiftChange, ResponseValue respValue) {
		...
		//交接班后，将交班麻醉医生后面的手术全部改成接班医生
		DocAnaesRecord docAnaesRecord = docAnaesRecordDao.searchAnaesRecordById(shiftChange.getDocId());
		if(null!=docAnaesRecord){
			String operRoomId = basDispatchDao.getCurRoomIdByRegOptId(docAnaesRecord.getRegOptId());
			BasRegOpt basRegOpt = basRegOptDao.searchRegOptById(docAnaesRecord.getRegOptId());
			if(null!=basRegOpt){
				List<BasDispatch> dispatchList = basDispatchDao.getBasDispatchListByAnaesDoc(operRoomId, basRegOpt.getOperaDate(),shiftChange.getShiftChangedPeopleId(), basRegOpt.getBeid());
				if(null != dispatchList && dispatchList.size() > 0){
					for (BasDispatch basDispatch : dispatchList) {
						basDispatch.setAnesthetistId(shiftChange.getShiftChangePeopleId());//接班人员
						basDispatchDao.update(basDispatch);
					}
				}
			}
