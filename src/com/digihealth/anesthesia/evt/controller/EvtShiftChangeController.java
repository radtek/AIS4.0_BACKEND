...
public class EvtShiftChangeController extends BaseController{
	 ...
	 * @return
	 */
	@RequestMapping(value = "/saveShiftChangeSYZXYY")
	@ResponseBody
	@ApiOperation(value = "保存交接班事件", httpMethod = "POST", notes = "保存交接班事件")
	public String saveShiftChangeSYZXYY(@ApiParam(name = "shiftChange", value = "交接班事件对象") @RequestBody EvtShiftChange shiftChange) {
		logger.info("begin saveShiftChangeSYZXYY");
		ResponseValue respValue = new ResponseValue();
		...
		evtShiftChangeService.saveShiftChangeSYZXYY(shiftChange,respValue);
		
		logger.info("end saveShiftChangeSYZXYY");
		return respValue.getJsonStr();
	}
