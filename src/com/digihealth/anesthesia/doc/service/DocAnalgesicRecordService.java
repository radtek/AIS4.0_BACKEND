...
public class DocAnalgesicRecordService extends BaseService {
	...
	public AnalgesicRecordFormBean getAnalgesicRecord(String regOptId){
		...
		if(analgesic!=null){
			...
			record.setAnalgesicPostFlup(docAnalgesicPostflupDao.getByanalgesicId(analgesic.getId()));
		}
		return record;
	}
	...
	public void saveAnalgesicRecord(AnalgesicRecordFormBean record) {
		...
		List<DocAnalgesicPostflup> analgesicPostFlups = record.getAnalgesicPostFlup();
		...
		if(null != analgesicMedicalInfoLs && analgesicMedicalInfoLs.size()>0){
...
//				analgesicMedicalInfo.setPatId(analgesicRecord.getRegOptId());
		...
		if(null != analgesicVisitLs && analgesicVisitLs.size()>0){
...
//				analgesicVisitInfo.setPatId(analgesicRecord.getRegOptId());
		...
		if(null != analgesicPostFlups && analgesicPostFlups.size()>0){
			docAnalgesicPostflupDao.deleteByanalgesicId(analgesicId);
			for (DocAnalgesicPostflup analgesicPostflup : analgesicPostFlups) {
//				analgesicPostflup.setPatId(analgesicRecord.getRegOptId());
				analgesicPostflup.setAnalgesicId(analgesicId);
				analgesicPostflup.setId(GenerateSequenceUtil.generateSequenceNo());
				docAnalgesicPostflupDao.insertSelective(analgesicPostflup);
			}
		}
		if(null != analgesicRecipeLs && analgesicRecipeLs.size()>0){
...
//				analgesicRecipe.setPatId(analgesicRecord.getRegOptId());
	...
	/**
	 * 
	 * @discription 麻醉科术后镇痛单 耒阳人民医院
	 */
	@Transactional
	public void saveAnalgesicRecordLYYY(AnalgesicRecordFormBean record) {
		DocAnalgesicRecord analgesicRecord = record.getAnalgesicRecord();
		List<DocAnalgesicMedicalInfo> analgesicMedicalInfoLs = record.getAnalgesicMedicalInfo();
		List<DocAnalgesicRecipe> analgesicRecipeLs = record.getAnalgesicRecipe();
		List<DocAnalgesicVisitInfo> analgesicVisitLs = record.getAnalgesicVisitInfo();
