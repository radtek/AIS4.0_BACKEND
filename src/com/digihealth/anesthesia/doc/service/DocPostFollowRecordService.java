...
public class DocPostFollowRecordService extends BaseService {
	...
	public PostFollowRecordFormBean getPostFollowRecord(String regOptId){
		...
		if(postFollowRecord!=null){
            ...
            record.setDocPostFollowYxrm(docPostFollowSituationDao.selectPostFollowYxrmByFollowId(postFollowId)); 
		}
		return record;
	}
...
//		postFollowRecord.setState(controller.getState());
		...
		DocPostFollowSituation docPostFollowYxrm = record.getDocPostFollowYxrm();
		if(null != docPostFollowYxrm)
		{
			DocPostFollowSituation old = docPostFollowSituationDao.selectPostFollowYxrmByFollowId(postFollowId);
			if(null != old)
			{
				docPostFollowYxrm.setPostFollowStId(old.getPostFollowStId());
				docPostFollowYxrm.setPostFollowId(postFollowId);
				docPostFollowSituationDao.updateByPrimaryKey(docPostFollowYxrm);
			}else
			{
				docPostFollowYxrm.setPostFollowStId(GenerateSequenceUtil.generateSequenceNo());
				docPostFollowYxrm.setPostFollowId(postFollowId);
				docPostFollowSituationDao.insertSelective(docPostFollowYxrm);
			}
		}
