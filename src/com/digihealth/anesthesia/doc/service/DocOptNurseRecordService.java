...
public class DocOptNurseRecordService extends BaseService {
	...
	public String searchOptNurseRecByRegOptId(Map<String, Object> map) {
		...
		// 根据患者id获取到手术基本信息
		SearchRegOptByIdFormBean searchRegOptByIdFormBean = basRegOptDao
	...
	private void saveEgress(DocOptNurseRecord optNurseRec) {
		...
		if (optNurseRec.getBleeding() != null) {
			...
			if (egressList1 != null && egressList1.size() > 0) {
				...
				egress1.setValue(null != optNurseRec.getBleeding() ? optNurseRec.getBleeding().floatValue() : null);
				evtEgressDao.updateByPrimaryKeySelective(egress1);
			} else {
				egress.setIoDefId("2");
				egress.setEgressId(GenerateSequenceUtil.generateSequenceNo());
				egress.setValue(null !=  optNurseRec.getBleeding() ? optNurseRec.getBleeding().floatValue() : null);
		...
		if (optNurseRec.getUrine() != null) {
			...
			if (egressList2 != null && egressList2.size() > 0) {
				...
				egress2.setValue(null != optNurseRec.getUrine() ? optNurseRec.getUrine().floatValue() : null);
				evtEgressDao.updateByPrimaryKeySelective(egress2);
			} else {
				egress.setEgressId(GenerateSequenceUtil.generateSequenceNo());
				egress.setIoDefId("1");
				egress.setValue(null != optNurseRec.getUrine() ? optNurseRec.getUrine().floatValue() : null);
				evtEgressDao.insert(egress);
			}
		}
		...
		if (optNurseRec.getHematocele() != null) {
			...
			if (egressList3 != null && egressList3.size() > 0) {
				...
				egress3.setValue(null != optNurseRec.getHematocele() ? optNurseRec.getHematocele().floatValue() : null);
				evtEgressDao.updateByPrimaryKeySelective(egress3);
			} else {
				egress.setEgressId(GenerateSequenceUtil.generateSequenceNo());
				egress.setIoDefId("3");
				egress.setValue(null != optNurseRec.getHematocele() ? optNurseRec.getHematocele().floatValue() : null);
				evtEgressDao.insert(egress);
			}
		}
		...
		if (optNurseRec.getOutOther() != null) {
			...
			if (egressList4 != null && egressList4.size() > 0) {
				...
				egress4.setValue(StringUtils.isNotBlank(optNurseRec.getOutOther()) ? Float.valueOf(optNurseRec.getOutOther()) : null);
				evtEgressDao.updateByPrimaryKeySelective(egress4);
			} else {
				egress.setEgressId(GenerateSequenceUtil.generateSequenceNo());
				egress.setIoDefId("6");
				egress.setValue(StringUtils.isNotBlank(optNurseRec.getOutOther()) ? Float.valueOf(optNurseRec.getOutOther()) : null);
				evtEgressDao.insert(egress);
			}
		}
	}
	...
	private void setIOEvent(DocAnaesRecord anaesRecord, DocOptNurseRecord optNurseRec) {
		...
		Integer bleeding = evtEgressDao.getEgressCountValueByIoDefName("出血量", anaRecordId, null) == null ? 0 : evtEgressDao.getEgressCountValueByIoDefName("出血量", anaRecordId, null);
		optNurseRec.setBleeding(bleeding); // 失血量
		Integer urine = evtEgressDao.getEgressCountValueByIoDefName("尿量", anaRecordId, null) == null ? 0 : evtEgressDao.getEgressCountValueByIoDefName("尿量", anaRecordId, null);
		optNurseRec.setUrine(urine); // 尿量
		Integer hematocele = evtEgressDao.getEgressCountValueByIoDefName("积血量", anaRecordId, null) == null ? 0 : evtEgressDao.getEgressCountValueByIoDefName("积血量", anaRecordId, null);
		optNurseRec.setHematocele(hematocele); // 出量其他
		Integer outOther = evtEgressDao.getEgressCountValueByIoDefName("其他", anaRecordId, null) == null ? 0 : evtEgressDao.getEgressCountValueByIoDefName("其他", anaRecordId, null);
		optNurseRec.setOutOther(String.valueOf(outOther)); // 出量其他
		Integer outfusion = evtEgressDao.getEgressCountValueByIoDefName(null, anaRecordId, null) == null ? 0 : evtEgressDao.getEgressCountValueByIoDefName(null, anaRecordId, null);
