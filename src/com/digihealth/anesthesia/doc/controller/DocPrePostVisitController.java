...
public class DocPrePostVisitController extends BaseController
{
    ...
    public String searchPrePostVisit(@ApiParam(name="map", value ="查询参数") @RequestBody Map<String, Object> map) {
        ...
        //设置默认术前访视人和术后访视人
        if (null != searchRegOptByIdFormBean && StringUtils.isNotBlank(searchRegOptByIdFormBean.getCircunurseId1()))
        {
            if (null == prePostVisit.getPreVisitorId())
            {
                prePostVisit.setPreVisitorId(searchRegOptByIdFormBean.getCircunurseId1());
            }
            if (null == prePostVisit.getPostVisitorId())
            {
                prePostVisit.setPostVisitorId(searchRegOptByIdFormBean.getCircunurseId1());
            }
        }
