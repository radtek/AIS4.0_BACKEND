...
public class BasUserController extends BaseController {
	...
	public String updateUser(@ApiParam(name = "user", value = "用户对象") @RequestBody BasUser user) {
		...
		//生成电子签名图片，并保存图片路径
		if(StringUtils.isNotBlank(user.getDzqm())){
			String filePath = request.getSession().getServletContext().getRealPath("/")	+ Global.getConfig("imageUpload.user.path");
			String httpPath = request.getLocalAddr() + ":" + request.getLocalPort();
			String fileName = user.getUserName()+".png";
			// 文件保存路径
			filePath += "/" + fileName;  
			// 保存图片路径
			user.setPicPath("http://" + httpPath + "/" + Global.getConfig("imageUpload.user.path")+ "/" + fileName);
			ImageUpload.GenerateImage(user.getDzqm(), filePath);
		}
