...
public class TmpSciResearchFieldService extends BaseService{
	...
	public List<SearchAnaesRegInfo> changeResearchAnalysisTemp(SciResearchFormBean bean){
		...
		sqlBuff.append("select DISTINCT a.regOptId,a.isLocalAnaes,operaDate,name,sex,age,ageMon,ageDay,hid,regionName,deptName,bed,designedOptName,designedAnaesMethodName,p.id pacuId "
				+ "From bas_reg_opt a left join doc_anaes_record b on a.regOptId = b.regOptId left join doc_anaes_pacu_rec p on a.regOptId = p.regOptId ");
		
		StringBuffer sqlWhere = new StringBuffer();
		
		sqlWhere.append(" where a.state in ('06','07')");
...
//			}
	...
	public SciCompareFormbean getSciCompareRegInfo(String regOptId){
		...
		record.setEmergency(basDictItemDao.searchSysCodeByGroupIdAndCodeValue("operat_category", regOpt.getEmergency()+"", getBeid()).size()>0?basDictItemDao.searchSysCodeByGroupIdAndCodeValue("operat_category", regOpt.getEmergency()+"", getBeid()).get(0).getCodeName():regOpt.getEmergency()+"");
		...
		if(StringUtils.isNotBlank(anesRecord.getOptBody())){
			...
			for (int i = 0; i < optBodyArr.length; i++) {
				optBody += basDictItemDao.searchSysCodeByGroupIdAndCodeValue("opt_body",optBodyArr[i], getBeid()).get(0).getCodeName()+",";
			}
		}
		...
		record.setAsa(StringUtils.isNotBlank(anesRecord.getAsaLevel())?basDictItemDao.searchSysCodeByGroupIdAndCodeValue("asa_level",anesRecord.getAsaLevel(), getBeid()).get(0).getCodeName():"");
	...
	public LifeSignFormBean getSplitObservesData(String regOptId,Integer pageSize){
		...
		baseQuery.setOperRoomId(getCurRoomId(regOptId));
		...
		if (null != anaesLists && anaesLists.size() > 0) {
			for (BasAnaesMonitorConfigFormBean fBean : anaesLists) {
                ...
                // 获取显示项需要的observeIds
                BasMonitorConfigDefault bmcf = basMonitorConfigDefaultDao.selectByEventName(observeName, getBeid(), 1);
                if (null != bmcf)
                {
		...
		List<BasMonitorDisplay> observeTimes = basMonitorDisplayDao.searchObserveTimeById(regOptId,observeIds, null);
		...
		if (observeTimes != null & total > 0) {
			...
			List<BasMonitorDisplay> resultList = basMonitorDisplayDao.searchObserveDataList(infoQry,observeIds, null);
	...
	public List<LifeSignObserveFormBean> getLifeSignList() {
		return basMonitorConfigDao.getLifeSignList(getBeid(),getCurRoomId(null));
	}
