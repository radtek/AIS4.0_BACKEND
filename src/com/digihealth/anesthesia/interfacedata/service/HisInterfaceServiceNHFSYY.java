...
public class HisInterfaceServiceNHFSYY
{
...
//        System.out.println(inBody);
	...
	public void getHisOperateNotice(){
        ...
        logger.info("---------------------begin synHisOperateNotice------------------------");
        Connection conn = null;
        
        SimpleDateFormat sim = new SimpleDateFormat("yyyyMMdd");
        String operDate = sim.format(new Date());
        
        String sql = "select * from VW_CX_SM_SSTZD where LEFT(operDate,8) >='" + operDate + "'";
        logger.info("=================sql=================" + sql);
        PreparedStatement pstmt = null;
        ResultSet rs = null;
        try {
            conn = ConnectionManager.getNHDXFSYYHisConnection();
            ...
            while (rs.next()) {
                ...
                String beid = basBusEntityDao.getBeid();
                int result = basRegOptDao.selectHisToRegOpt(rs.getString("reservenumber"), DateUtils.strToStr(rs.getString("operDate"), "yyyyMMddHH:mm", "yyyy-MM-dd"), beid);
                if(result<1){
                    ...
                    if(null != rs.getString("birthday")){
                    	String birthday = DateUtils.strToStr(rs.getString("birthday").trim(), "yyyyMMdd", "yyyy-MM-dd");
                    	logger.info("synHisOperList=============birthday:"+birthday+"===============");
                    ...
                    if (StringUtils.isNotEmpty(rs.getString("diagCode")))
                    {
                        ...
                        {
                            BasDiagnosedef diagnosedefs = basDiagnosedefDao.selectByCode(diagCode[i], beid);
                            if (null != diagnosedefs)
                            {
                                diagDef = diagDef + diagnosedefs.getDiagDefId() + ",";
                                diagName = diagName + diagnosedefs.getName() + ",";
                            }
                    ...
                    if (StringUtils.isNotEmpty(rs.getString("operCode")))
                    {
                        ...
                        {
                            BasOperdef opers = basOperdefDao.selectByCode(operCodes[i], beid);
                            if (null != opers)
                            {
                                operId = operId + opers.getOperdefId() + ",";
                                operName = operName + opers.getName() + ",";
                            }
                    ...
                    if (StringUtils.isNotEmpty(rs.getString("operatorId")))
                    {
                        String operatorId = "";
                        String operatorName = "";
                        String[] operatorCodes = rs.getString("operatorId").split(",");
                        for (int i = 0; i < operatorCodes.length; i++)
                        {
                            BasOperationPeople operators = basOperationPeopleDao.selectByCode(operatorCodes[i], beid);
                            if (null != operators)
                            {
                                operatorId = operatorId + operators.getOperatorId() + ",";
                                operatorName = operatorName + operators.getName() + ",";
                            }
                            else if (StringUtils.isNotBlank(rs.getString("operatorName")))
                            {
                                String[] operatorNames = rs.getString("operatorName").split(",");
                    ...
                    regOpt.setOperatorName(rs.getString("operatorName"));
                    ...
                    regOpt.setOperaDate(DateUtils.strToStr(rs.getString("operDate"), "yyyyMMddHH:mm", "yyyy-MM-dd"));
...
//                   regOpt.setHyperSusceptiBility(rs.getString("dragAllergy"));
                    ...
                    if(!StringUtils.isEmpty(optLevel)){
	                    if("一类手术".equals(optLevel)){
	                    	regOpt.setOptLevel("1");
	                    }
	                    if("二类手术".equals(optLevel)){
	                    	regOpt.setOptLevel("2");
	                    }
	                    if("三类手术".equals(optLevel)){
	                    	regOpt.setOptLevel("3");
	                    }
	                    if("四类手术".equals(optLevel)){
	                    	regOpt.setOptLevel("4");
	                    }
	                    if("五类手术".equals(optLevel)){
	                    	regOpt.setOptLevel("5");
	                    }
	                    if("六类手术".equals(optLevel)){
	                    	regOpt.setOptLevel("6");
	                    }
                    }
                    ...
                    if(!StringUtils.isEmpty(rs.getString("operType"))){
                    	Integer emergency= 0;
                    	//普通、急诊、择期
                    	if("急诊".equals(rs.getString("operType"))){
                    		emergency = 1;
                    	}
            			regOpt.setEmergency(emergency);
            		}
...
//                            operatelogService.OPT_MODULE_INTERFACE, "HIS手术通知单", JsonType.jsonType(regOpt));
            ...
            logger.info("synHisOperateNotice=============while end===============");
        } catch (Exception e) {
            logger.error("synHisOperateNotice--------------"+Exceptions.getStackTraceAsString(e));
        }
        finally
        {
            try
        ...
        logger.info("------------------end synHisOperateNotice-------------------------");
