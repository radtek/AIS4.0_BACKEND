...
public class BasRegOptService extends BaseService {
    ...
    public String updateRegOptInfo(BasRegOpt regOpt, BasDispatch dispatch,ResponseValue resp) {
		...
		if(null!=oldOpt && OperationState.SURGERY.equals(oldOpt.getState())){
			...
			if ((Objects.equals(1, oldOpt.getIsLocalAnaes()) && Objects.equals(0, regOpt.getIsLocalAnaes())) ||
					(Objects.equals(0, regOpt.getIsLocalAnaes()) && !oldDispatch.getOperRoomId().equals(dispatch.getOperRoomId()))){
				...
				if(operaPatrolList.size()>0){
					for (SearchOperaPatrolRecordFormBean searchOperaPatrolRecordFormBean : operaPatrolList) {
						if(Objects.equals(0, searchOperaPatrolRecordFormBean.getIsLocalAnaes())){
			...
			//更换手术室后将原手术室采集服务停止
			if(Objects.equals(0, regOpt.getIsLocalAnaes()) && !oldDispatch.getOperRoomId().equals(dispatch.getOperRoomId())){
			    ...
			    BasCollectConfig oldCollectConfig = basCollectConfigDao.selectByPrimaryKey(oldDispatch.getOperRoomId(), getBeid());
                if (null != oldCollectConfig)
                {
                    oldCollectConfig.setPatientId(null);
                    basCollectConfigDao.updateByPrimaryKey(oldCollectConfig);
                }
                
                BasCollectConfig collectConfig = basCollectConfigDao.selectByPrimaryKey(dispatch.getOperRoomId(), getBeid());
                if (null != collectConfig)
                {
                    collectConfig.setPatientId(regOpt.getRegOptId());
			...
			if (!Objects.equals(oldOpt.getIsLocalAnaes(), regOpt.getIsLocalAnaes())) {
			 ...
			 */
			if(Objects.equals(0, regOpt.getIsLocalAnaes())){
				SearchFormBean searchBean = new SearchFormBean();
				DocAnaesRecord anaesRecord = docAnaesRecordDao.searchAnaesRecordByRegOptId(regOpt.getRegOptId());
		...
		//如果为局麻手术，先判断是否需要麻醉医生，如不需要则需要删除participant表对应的麻醉医生信息。
		if(Objects.equals(1, regOpt.getIsLocalAnaes())){
			String isNeedDoct = CustomConfigureUtil.isNeedDoct();
			if(!"1".equals(isNeedDoct)){
				SearchFormBean searchBean = new SearchFormBean();
				searchBean.setRole(EvtParticipantService.ROLE_ANESTH);
		        List<EvtParticipant> anesDocList = evtParticipantDao.searchParticipantList(searchBean, beid);
		        for (EvtParticipant evtParticipant : anesDocList) {
		        	evtParticipantDao.deleteById(evtParticipant);
				}
			}
		}
        ...
        if (null != regOpt && 1 == regOpt.getEmergency() && "02".equals(regOpt.getState()))
       {
        ...
        if (null != regOpt && 
            ...
            && Objects.equals(0, regOpt.getIsLocalAnaes())) 
        {
            SearchFormBean searchBean = new SearchFormBean();
            DocAnaesRecord anaesRecord = docAnaesRecordDao.searchAnaesRecordByRegOptId(regOpt.getRegOptId());
            String docId = anaesRecord.getAnaRecordId();
	...
	public void suspendOperation(SuspendFormBean bean) {
		...
		//将采集中间表中的手术Id清除掉
		BasCollectConfig collectConfig = basCollectConfigDao.selectByPrimaryKey(getCurRoomId(null), getBeid());
