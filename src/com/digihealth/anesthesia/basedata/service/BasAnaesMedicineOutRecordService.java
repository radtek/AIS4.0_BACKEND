...
import com.digihealth.anesthesia.basedata.formbean.BatAddAnaesMedicineOutRecordFormbean;
import com.digihealth.anesthesia.basedata.formbean.BatAddAnaesMedicineRetreatFormbean;
...
public class BasAnaesMedicineOutRecordService extends BaseService
{
	...
	@Transactional
	public void batAddAnaesMedicineOutRecord(BatAddAnaesMedicineOutRecordFormbean batRecord,ResponseValue resp)
	{
		List<BasAnaesMedicineOutRecord> resList = batRecord.getResList();
		if(resList!=null && resList.size()>0){
			for (BasAnaesMedicineOutRecord basAnaesMedicineOutRecord : resList) {
				Integer id = basAnaesMedicineOutRecord.getId();
				int outNumber = basAnaesMedicineOutRecord.getOutNumber();
				BasAnaesMedicineStorage basAnaesMedicineStorage = basAnaesMedicineStorageDao.selectByPrimaryKey(id);
				if(null != basAnaesMedicineStorage)
				{
					int number = basAnaesMedicineStorage.getNumber();
					if(outNumber>number)
					{
						resp.setResultCode("10000002");
			            resp.setResultMessage(basAnaesMedicineOutRecord.getMedicineName()+"取药的数量不能大于库存的数量");
			            throw new CustomException(basAnaesMedicineOutRecord.getMedicineName()+"取药的数量不能大于库存的数量");
					}else
					{
						String outType = batRecord.getOutType();
						String regOptId = basAnaesMedicineOutRecord.getRegOptId();
						basAnaesMedicineOutRecord.setStorageId(id);
						basAnaesMedicineOutRecord.setId(null);
						basAnaesMedicineOutRecord.setActualNumber(outNumber);
						basAnaesMedicineOutRecord.setOutTime(new Date());
						basAnaesMedicineOutRecord.setReceiveName(batRecord.getReceiveName());
						basAnaesMedicineOutRecord.setPinYin(PingYinUtil.getFirstSpell(basAnaesMedicineOutRecord.getMedicineName()));
						basAnaesMedicineOutRecord.setReceiveName(batRecord.getReceiveName());
						basAnaesMedicineOutRecord.setOperator(batRecord.getOperator());
						basAnaesMedicineOutRecord.setOutType(outType);
						basAnaesMedicineOutRecordDao.insertSelective(basAnaesMedicineOutRecord);
						//如果是手术取药，更新手术信息表outMedicine字段
						if("2".equals(outType))
						{
				...
				{
					resp.setResultCode("10000003");
		            resp.setResultMessage(basAnaesMedicineOutRecord.getMedicineName()+"传输的库存记录不存在！");
		            throw new CustomException(basAnaesMedicineOutRecord.getMedicineName()+"传输的库存记录不存在！");
				}
			}
		}
	}
	...
	//增加毒麻药退药记录
	@Transactional
	public void batAddMedicineRetreatRecord(BatAddAnaesMedicineRetreatFormbean retreatRecord,ResponseValue resp)
	{
		List<BasAnaesMedicineRetreatRecord> resList = retreatRecord.getResList();
		for (BasAnaesMedicineRetreatRecord basAnaesMedicineRetreatRecord : resList) {
			if(StringUtils.isBlank(retreatRecord.getRetreatType())){
				retreatRecord.setRetreatType("1");//如果为空则默认为普通退药
			}
			//取得取药记录id
			int id = basAnaesMedicineRetreatRecord.getId();
			//int outRecordId = basAnaesMedicineRetreatRecord.getOutRecordId();
			int retreatNumber = basAnaesMedicineRetreatRecord.getRetreatNumber();
			//通过取药记录id获得取药记录信息
			...
			if(null != basAnaesMedicineOutRecord)
			{
				...
				{
					resp.setResultCode("10000002");
		            resp.setResultMessage(basAnaesMedicineRetreatRecord.getMedicineName()+"退药的数量不能大于取药的实际数量");
		            throw new CustomException(basAnaesMedicineRetreatRecord.getMedicineName()+"退药的数量不能大于取药的实际数量"); 
				}else
				{
					//退药记录存入数据库
					basAnaesMedicineRetreatRecord.setRetreatTime(new Date());
					basAnaesMedicineRetreatRecord.setRetreatName(retreatRecord.getRetreatName());
					basAnaesMedicineRetreatRecord.setRetreatreason(retreatRecord.getRetreatreason());
					basAnaesMedicineRetreatRecord.setOperator(retreatRecord.getOperator());
					basAnaesMedicineRetreatRecord.setOutRecordId(id);
					basAnaesMedicineRetreatRecord.setId(null);
					basAnaesMedicineRetreatRecord.setRetreatType(retreatRecord.getRetreatType());
					basAnaesMedicineRetreatRecordDao.insertSelective(basAnaesMedicineRetreatRecord);
					//将取药记录进行更新。更新退药数量和实际数量
					int outReteatNum = basAnaesMedicineOutRecord.getRetreatNumber();
					basAnaesMedicineOutRecord.setRetreatNumber(outReteatNum + retreatNumber);
			...
			{
				resp.setResultCode("10000003");
	            resp.setResultMessage(basAnaesMedicineRetreatRecord.getMedicineName()+"传输的取药记录不存在！");
	            throw new CustomException(basAnaesMedicineRetreatRecord.getMedicineName()+"传输的取药记录不存在！"); 
			}
		}
	}
