...
public class BasRegionBedService extends BaseService {
	...
	public int updateRegionBed(BasRegionBed regionBed) {
		...
		if (null != regionBed.getId()) {
		    BasCollectConfig collectConfig = basCollectConfigDao.selectByPrimaryKey(regionBed.getId(), regionBed.getBeid());
		    if (null == collectConfig)
		    {
		        collectConfig = new BasCollectConfig();
		        collectConfig.setRoomId(regionBed.getId());
		        collectConfig.setIp(regionBed.getIpAddr());
		        collectConfig.setBeid(regionBed.getBeid());
		        basCollectConfigDao.insert(collectConfig);
		    }
		    else if (!regionBed.getIpAddr().equals(collectConfig.getIp()))
		    {
		        collectConfig.setIp(regionBed.getIpAddr());
    		...
    		List<BasDeviceConfig> devList = basDeviceConfigDao.getDeviceConfigList(getBeid(), "00");
            for (BasDeviceConfig basDeviceConfig : devList)
            {
                basDeviceConfig.setRoomId(regionBed.getId());
