...
@Service
public class BasConsumablesLoseRecordService extends BaseService
{
	//查询耗材取药记录列表
	public List<BasConsumablesLoseRecordFormBean>  queryConsumablesLoseRecordList(SystemSearchFormBean systemSearchFormBean)
	{
		...
		return basConsumablesLoseRecordDao.queryConsumablesLoseRecordList(
				filters, systemSearchFormBean);
	}
	
	//查询耗材入库信息数量
	public int queryConsumablesLoseRecordListTotal(SystemSearchFormBean systemSearchFormBean)
	{
		List<Filter> filters = systemSearchFormBean.getFilters();
		return basConsumablesLoseRecordDao.queryConsumablesLoseRecordListTotal(filters);
	}
     ...
     * @return
     */
    public List<StatisticsFormbean> queryConsumablesLoseRecordForLine(String startDt,String endDt,String queryType){
        ...
        if(queryType.equals("1")){
            ...
            if(month>=1){
                for (int i = 1; i <= month; i++) {
                    ...
                    record.setName(i+"月");
                    Integer loseNum = basConsumablesLoseRecordDao.queryConsumablesLoseRecordForLine(startTime, endTime, getBeid());
    ...
    //根据原因查询报损数量
    public Integer queryConsumablesLoseRecordByReason(String startTime,String endTime,String reason)
    {
    	return basConsumablesLoseRecordDao.queryConsumablesLoseRecordByReason(startTime, endTime, reason, getBeid());
    }
}
