...
@Api(value = "BasConsumablesLoseRecordController", description = "耗材报损处理类")
public class BasConsumablesLoseRecordController extends BaseController
{
	/**
     * 
     * @discription 根据条件查询耗材报损记录
     * @author pengqing
     * @created 2017年6月13日 下午3:01:40
     * @param systemSearchFormBean
     * @return
     */
    @RequestMapping(value = "/queryConsumablesLoseRecordList")
    @ResponseBody
    @ApiOperation(value = "根据条件查询耗材取药记录", httpMethod = "POST", notes = "根据条件查询耗材取药记录")
    public String queryConsumablesLoseRecordList(
        @ApiParam(name = "systemSearchFormBean", value = "系统查询参数") @RequestBody SystemSearchFormBean systemSearchFormBean)
    {
        logger.info("begin queryConsumablesLoseRecordList");
        ResponseValue resp = new ResponseValue();
        List<BasConsumablesLoseRecordFormBean> resultList = basConsumablesLoseRecordService.queryConsumablesLoseRecordList(systemSearchFormBean);
        int total = basConsumablesLoseRecordService.queryConsumablesLoseRecordListTotal(systemSearchFormBean);
        resp.put("resultList", resultList);
        resp.put("total", total);
        logger.info("end queryConsumablesLoseRecordList");
	 ...
	 * 报损记录,通过折线图展示
	 */
	@RequestMapping("/queryConsumablesLoseRecordForLine")
	@ResponseBody
	@ApiOperation(value="报损记录,通过折线图展示",httpMethod="POST",notes="报损记录,通过折线图展示")
    public String queryConsumablesLoseRecordForLine(@ApiParam(name="map", value ="系统查询参数对象") @RequestBody Map<String,Object> map) 
	{
		ResponseValue resp = new ResponseValue();
		...
		List<StatisticsFormbean> list = basConsumablesLoseRecordService
				.queryConsumablesLoseRecordForLine(stTime, edTime, queryType);
	 ...
	 */
	@RequestMapping("/queryConsumablesLoseRecordForPie")
	@ResponseBody
	@ApiOperation(value="报损记录 按照报损原因（霉变、破损、失效、其他）进行平图分层",httpMethod="POST",notes="报损记录 按照报损原因（霉变、破损、失效、其他）进行平图分层")
    public String queryConsumablesLoseRecordForPie(@ApiParam(name="map", value ="系统查询参数对象") @RequestBody Map<String,Object> map) 
	{
    	ResponseValue resp = new ResponseValue();
    	String queryType = map.get("queryType").toString();
		String stTime = map.get("startTime").toString();
		...
		Integer mbNum = basConsumablesLoseRecordService.queryConsumablesLoseRecordByReason(startTime,endTime,"霉变");
		Integer psNum = basConsumablesLoseRecordService.queryConsumablesLoseRecordByReason(startTime,endTime,"破损");
		Integer sxNum = basConsumablesLoseRecordService.queryConsumablesLoseRecordByReason(startTime,endTime,"失效");
		Integer qtNum = basConsumablesLoseRecordService.queryConsumablesLoseRecordByReason(startTime,endTime,"其他");
