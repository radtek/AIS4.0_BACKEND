...
public class StatisticsController extends BaseController {
	...
	public String searchMedIoCharge(@RequestBody SystemSearchFormBean systemSearchFormBean) {
		...
		if(filters!=null&&filters.size()>0){
			for (Filter fi : filters) {
				if(!StringUtils.isEmpty(fi.getValue())){
					if("regOptId".equals(fi.getField())){
						regOptId = fi.getValue();
					}
					if("costType".equals(fi.getField())){
						costType = fi.getValue();
					}
				}
			}
		}
        // 麻醉费用
		float anaesChargeShouldSum = 0.0f;
        List<DocPackagesItem> anaesChargeList = docPackagesItemService.queryPackagesItemList(systemSearchFormBean, 1);
        for (DocPackagesItem packagesItem : anaesChargeList) {
        	//anaesChargeShouldSum = anaesChargeShouldSum.add(new BigDecimal(packagesItem.getShouldSum()));
        	anaesChargeShouldSum = Arith.add(anaesChargeShouldSum, packagesItem.getShouldSum());
		}
        
        // 材料费用
        float materialShouldSum = 0.0f;
        List<DocPackagesItem> materialList = docPackagesItemService.queryPackagesItemList(systemSearchFormBean, 2 );
        for (DocPackagesItem packagesItem : materialList) {
			//materialShouldSum = materialShouldSum.add(new BigDecimal(packagesItem.getShouldSum()));
        	materialShouldSum = Arith.add(materialShouldSum, packagesItem.getShouldSum());
		}
        
        //麻醉操作费用
        float anaesOptShouldSum = 0.0f;
        List<DocPackagesItem> anaesOptList = docPackagesItemService.queryPackagesItemList(
                systemSearchFormBean, 3);
        for (DocPackagesItem packagesItem : materialList) {
        	anaesOptShouldSum = Arith.add(anaesOptShouldSum, packagesItem.getShouldSum());
        	//anaesOptShouldSum = anaesOptShouldSum.add(new BigDecimal(packagesItem.getShouldSum()));
		}
        
        
        float medShouldSum = 0.0f;
		// 药品
		...
		medsystemSearchFormBean.setSort("ebId");
		medsystemSearchFormBean.setOrderBy("asc");
		...
		if(null!=medList && medList.size()>0){
			for (EventBillingFormBean eventBillingFormBean : medList) {
				medShouldSum = Arith.add(medShouldSum,eventBillingFormBean.getShouldCost());
				eventBillingFormBean.setCostType(costType);
			}
		}
		
		float ioShouldSum = 0.0f;
		// 入量
		...
		if(null!=ioList && ioList.size()>0){
			for (EventBillingFormBean eventBillingFormBean2 : ioList) {
				ioShouldSum = Arith.add(ioShouldSum,eventBillingFormBean2.getShouldCost());
				eventBillingFormBean2.setCostType(costType);
			}
		}
		...
		//手术室名称
		String operRoomName = basOperroomService.selectEntityByPrimaryKey(dispatch.getOperRoomId().toString()).getName();
		...
		String instrnurse1="",instrnurse2="";
		if(null!=dispatchBean){
			instrnurse1 = dispatchBean.getInstrnurseName1();
			instrnurse2 = dispatchBean.getInstrnurseName2();
		}
		...
		map.put("anaesOptList", anaesOptList);
		map.put("ioList", ioList);
		//map.put("packagesCharge", packagesCharge);
		map.put("anaesChargeList", anaesChargeList);
        ...
        map.put("anaesChargeShouldSum", anaesChargeShouldSum);
        map.put("medShouldSum", medShouldSum);
        map.put("ioShouldSum", ioShouldSum);
        map.put("anaesOptShouldSum", anaesOptShouldSum);
	...
	public String updateMedIoCharge(
		...
		try {
			...
			String regOptId = "";
			String costType = "";
			List<DocEventBilling> ckList = new ArrayList<DocEventBilling>();
			if(null!=medIoChargeFormBean.getMedList() && medIoChargeFormBean.getMedList().size()>0)
				ckList.addAll(medIoChargeFormBean.getMedList());
			if(null!=medIoChargeFormBean.getIoList() && medIoChargeFormBean.getIoList().size()>0)
				ckList.addAll(medIoChargeFormBean.getIoList());
			if(StringUtils.isBlank(regOptId) && ckList!=null && ckList.size()>0){
				regOptId = ckList.get(0).getRegOptId();
				costType = ckList.get(0).getCostType();
			}else{
				List<DocPackagesItem> pkList = new ArrayList<DocPackagesItem>();
				if(null!=medIoChargeFormBean.getAnaesChargeList() && medIoChargeFormBean.getAnaesChargeList().size()>0)
					pkList.addAll(medIoChargeFormBean.getAnaesChargeList());
				if(null!=medIoChargeFormBean.getMaterialList() && medIoChargeFormBean.getMaterialList().size()>0)
					pkList.addAll(medIoChargeFormBean.getMaterialList());
				if(null!=medIoChargeFormBean.getAnaesOptList() && medIoChargeFormBean.getAnaesOptList().size()>0)
					pkList.addAll(medIoChargeFormBean.getAnaesOptList());
				
				if(StringUtils.isBlank(regOptId) && pkList!=null && pkList.size()>0){
					regOptId = pkList.get(0).getRegOptId();
					costType = pkList.get(0).getCostType();
				}
			}
			...
			if(StringUtils.isNotBlank(regOptId)){
				...
				if(!modFlag){
					map.put("resultCode", resp.getResultCode());
					map.put("resultMessage", resp.getResultMessage());
					return JsonType.jsonType(map);
				}
			}
			...
			String result = statisticsService
					.updateMedIoCharge(medIoChargeFormBean,regOptId,costType);
			map.put("resultCode", "1");
...
//            }
	...
	* 意外死亡、进入麻醉复苏室例数统计按麻醉医生
     ...
     * @return
     */
    @RequestMapping(value = "/searchAnaesEffectCountByAnaesMethod")
    @ResponseBody
    @ApiOperation(value="麻醉效果统计",httpMethod="POST",notes="麻醉效果统计")
    public String searchAnaesEffectCountByAnaesMethod(@ApiParam(name="searchConditionFormBean", value ="统计查询参数") @RequestBody SearchFormBean searchFormBean) 
    {
        logger.info("begin searchAnaesEffectCountByAnaesMethod");
        ResponseValue resValue = new ResponseValue();
        if (StringUtils.isNotBlank(searchFormBean.getBeid()))
        {
            searchFormBean.setBeid(getBeid());
        }
        
        List<SearchDeptOperatCountByAsalev> list = statisticsService.searchAnaesEffectCountByAnaesMethod(searchFormBean);
        String[] time = new String[list.size()];
        List<BasDictItem> fieldList = basDictItemService.getListByGroupId("anaes_effect_level", null, searchFormBean.getBeid()); 
        ...
        for (SearchDeptOperatCountByAsalev searchDeptOperatCountByAsalev : list) 
        {
            sdata.add(searchDeptOperatCountByAsalev.getAnaMedName());
        ...
        for (BasDictItem dicItem : fieldList) {
            Map map = new HashMap();
            
            Series series = new Series();
            String field = "";
            String name = "";
            if("1".equals(dicItem.getCodeValue())){
                field = "fst";
                name = "一级";
                series.setData(fstData);
            }
            if("2".equals(dicItem.getCodeValue())){
                field = "sec";
                name = "二级";
                series.setData(secData);
            }
            if("3".equals(dicItem.getCodeValue())){
                field = "thd";
                name = "三级";
                series.setData(thdData);
            }
            if("4".equals(dicItem.getCodeValue())){
                field = "fou";
                name = "四级";
                series.setData(fouData);
            }
        ...
        logger.info("end searchAnaesEffectCountByAnaesMethod");
        return resValue.getJsonStr();
    }
}
