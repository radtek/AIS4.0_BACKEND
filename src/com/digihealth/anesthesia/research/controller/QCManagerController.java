...
public class QCManagerController extends BaseController{
        		...
        		if(extList.size()>0){
        			...
        			for (DocPostFollowSpinal postFollowSpinal : extList) {
        				if((null == postFollowSpinal.getCognitObstacle() || postFollowSpinal.getCognitObstacle()==0) 
        				    ...
        				    if(Objects.equals(postFollowSpinal.getSevereHeadache(), 1)){
                                contents+="重度头痛,";
                            }
                            if(Objects.equals(postFollowSpinal.getLeftLimbsFeelImp(), 1)){
                                contents+="麻木,";
                            }
                            if(Objects.equals(postFollowSpinal.getRightLimbsFeelImp(),1)){
                                contents+="异感,";
                            }
                            if(Objects.equals(postFollowSpinal.getLeftMoveFeelImp(),1)){
                                contents+="肌无力,";
                            }
                            if(Objects.equals(postFollowSpinal.getRightMoveFeelImp(),1)){
                                contents+="瘫痪,";
                            }
     ...
     * @see [类、类#方法、类#成员]
     */
    @RequestMapping(value = "/searchDoctorPatientRateSYZXYY")
    @ResponseBody
    @ApiOperation(value="麻醉科医患比",httpMethod="POST",notes="麻醉科医患比")
    public String searchDoctorPatientRateSYZXYY(@ApiParam(name="searchConditionFormBean", value ="统计查询参数") @RequestBody SearchConditionFormBean searchConditionFormBean)
    {
        logger.info("begin searchDoctorPatientRate");
        ResponseValue resp = new ResponseValue();
        
        String year = searchConditionFormBean.getStartTime();
        String month = searchConditionFormBean.getEndTime();
        
        SearchFormBean searchFormBean = new SearchFormBean();
        searchFormBean.setStartTime(DateUtils.DateToString(DateUtils.getFirstDayOfMonth(Integer.valueOf(year), Integer.valueOf(month) - 1)));
        searchFormBean.setEndTime(DateUtils.DateToString(DateUtils.getLastDayOfMonth(Integer.valueOf(year), Integer.valueOf(month) - 1)));
        if (StringUtils.isBlank(searchConditionFormBean.getBeid())) {
            searchFormBean.setBeid(getBeid());
        }else {
            searchFormBean.setBeid(searchConditionFormBean.getBeid());
        ...
        // 在岗麻醉医师总数
        BasAnaesDoctorAmount anaesDoctorAmount = basAnaesDoctorAmountService.selectByPrimaryKey(year + "-" + month, searchFormBean.getBeid());
        int anestheDocNum = (null == anaesDoctorAmount || null == anaesDoctorAmount.getAmount()) ? 0 : anaesDoctorAmount.getAmount();
        int operTotal = 0;
        ...
        resp.put("columnAry", COLUMN);
        resp.put("rate", rate);
        logger.info("end searchDoctorPatientRate");
        return resp.getJsonStr();
    }
     ...
     * @see [类、类#方法、类#成员]
     */
    @RequestMapping(value = "/searchASALevelRateSYZXYY")
    @ResponseBody
    @ApiOperation(value="各ASA分级麻醉患者比例",httpMethod="POST",notes="各ASA分级麻醉患者比例")
    public String searchASALevelRateSYZXYY(@ApiParam(name="searchConditionFormBean", value ="统计查询参数") @RequestBody SearchConditionFormBean searchConditionFormBean)
    {
        logger.info("begin searchASALevelRate");
        ResponseValue resp = new ResponseValue();
        ...
        {
            searchFormBean.setAsaLevel(i + "");
            List<SearchOperFormBean> asaLevelList = qcManagerService.searchAnaesQualityControlByType("1", searchFormBean);
            map.put(i == 6 ? "其他" : (i  + "级"), 0);
            if (asaLevelList != null && asaLevelList.size() > 0)
            {
                map.put(i == 6 ? "其他" : (i  + "级"), asaLevelList.size());
                for (int j = 0; j < asaLevelList.size(); j++)
                {
                    vo = new HashMap<String, String>();
                    vo.put("级别", i == 6 ? "其他" : (i  + "级"));
                    vo.put(COLUMN[0], asaLevelList.get(j).getName());
                    vo.put(COLUMN[1], asaLevelList.get(j).getSex());
                    vo.put(COLUMN[2], asaLevelList.get(j).getAge());
                    vo.put(COLUMN[3], asaLevelList.get(j).getHid());
     ...
     * @see [类、类#方法、类#成员]
     */
    @RequestMapping(value = "/searchEmergencyOperSYZXYY")
    @ResponseBody
    @ApiOperation(value="急诊非择期麻醉比例",httpMethod="POST",notes="急诊非择期麻醉比例")
    public String searchEmergencyOperSYZXYY(@ApiParam(name="searchConditionFormBean", value ="统计查询参数") @RequestBody SearchConditionFormBean searchConditionFormBean)
    {
        logger.info("begin searchEmergencyOper");
        ResponseValue resp = new ResponseValue();
        SearchFormBean searchFormBean = new SearchFormBean();
        ...
        int emergencyTotal = 0;
        List<SearchOperFormBean> emergencyList = qcManagerService.searchAnaesQualityControlByType("2", searchFormBean);
     ...
     * @see [类、类#方法、类#成员]
     */
    @RequestMapping(value = "/searchAnesthMethodRateSYZXYY")
    @ResponseBody
    @ApiOperation(value="各类麻醉方式比例",httpMethod="POST",notes="各类麻醉方式比例")
    public String searchAnesthMethodRateSYZXYY(@ApiParam(name="searchConditionFormBean", value ="统计查询参数") @RequestBody SearchConditionFormBean searchConditionFormBean) {
        logger.info("begin searchAnesthMethodRate");
        ResponseValue resp = new ResponseValue();
        SearchFormBean searchFormBean = new SearchFormBean();
        searchFormBean.setStartTime(searchConditionFormBean.getStartTime());
        ...
        String[] anesTypeName = {"椎管内麻醉","插管全麻","非插管全麻","复合麻醉","其他麻醉方式"};
        
        for (int i = 0; i < 5; i++)
        {
            searchFormBean.setAnesType((i + 1) + "");
            List<SearchOperFormBean> anesTypeList = qcManagerService.searchAnaesQualityControlByType("3", searchFormBean); 
            map.put(anesTypeName[i], anesTypeList.size());
            for (int j = 0; j < anesTypeList.size(); j++) {
                vo = new HashMap<String, String>();
                vo.put("麻醉方法", anesTypeName[i]);
                vo.put(AMCOLUMN[7], anesTypeName[i]);
                vo.put(AMCOLUMN[0], anesTypeList.get(j).getName());
                vo.put(AMCOLUMN[1], anesTypeList.get(j).getSex());
                vo.put(AMCOLUMN[2], anesTypeList.get(j).getAge());
                vo.put(AMCOLUMN[3], anesTypeList.get(j).getHid());
                vo.put(AMCOLUMN[4], anesTypeList.get(j).getOperaDate());
                vo.put(AMCOLUMN[5], anesTypeList.get(j).getOperatorName());
                vo.put(AMCOLUMN[6], anesTypeList.get(j).getDesignedOptName());
                vo.put(AMCOLUMN[8], anesTypeList.get(j).getAnesthetistName());
                vo.put(AMCOLUMN[9], anesTypeList.get(j).getCircunurseName());
     ...
     * @see [类、类#方法、类#成员]
     */
    @RequestMapping(value = "/searchOperCancleRateSYZXYY")
    @ResponseBody
    @ApiOperation(value="麻醉开始后手术取消率",httpMethod="POST",notes="麻醉开始后手术取消率")
    public String searchOperCancleRateSYZXYY(@ApiParam(name="searchConditionFormBean", value ="统计查询参数") @RequestBody SearchConditionFormBean searchConditionFormBean)
    {
        logger.info("begin searchOperCancleRate");
        ResponseValue resp = new ResponseValue();
        ...
        List<SearchOperFormBean> operList = qcManagerService.searchOperByCondition(searchFormBean);
        List<SearchOperFormBean> operCancleList = qcManagerService.searchAnaesQualityControlByType("4", searchFormBean); 
    ...
    @RequestMapping(value = "/searchAnaesPacuDelayRateSYZXYY")
    @ResponseBody
    @ApiOperation(value="查询麻醉后监测PACU转出延迟率",httpMethod="POST",notes="查询麻醉后监测PACU转出延迟率")
    public String searchAnaesPacuDelayRateSYZXYY(@ApiParam(name="searchConditionFormBean", value ="查询参数") @RequestBody SearchConditionFormBean searchConditionFormBean) {
        logger.info("begin searchAnaesPacuDelayRate");
        ResponseValue resp = new ResponseValue();
        SearchFormBean searchFormBean = new SearchFormBean();
        searchFormBean.setStartTime(searchConditionFormBean.getStartTime());
        ...
        List<SearchOperFormBean> operList = qcManagerService.searchEnterPacuNum(searchFormBean);
        List<SearchOperFormBean> pacuDelayList = qcManagerService.searchAnaesPacuQualityControlByType("1", searchFormBean);
    ...
    @RequestMapping(value = "/searchAnaesPacuLowTempRateSYZXYY")
    @ResponseBody
    @ApiOperation(value="查询PACU入室低温率",httpMethod="POST",notes="查询PACU入室低温率")
    public String searchAnaesPacuLowTempRateSYZXYY(@ApiParam(name="searchConditionFormBean", value ="查询参数") @RequestBody SearchConditionFormBean searchConditionFormBean)
    {
        logger.info("begin searchAnaesPacuLowTempRate");
        ResponseValue resp = new ResponseValue();
        SearchFormBean searchFormBean = new SearchFormBean();
        ...
        List<SearchOperFormBean> operList = qcManagerService.searchEnterPacuNum(searchFormBean);
        List<SearchOperFormBean> pacuLowTmepList = qcManagerService.searchAnaesPacuQualityControlByType("2", searchFormBean);
     ...
     * @see [类、类#方法、类#成员]
     */
    @RequestMapping(value = "/searchNonPlanToICURateSYZXYY")
    @ResponseBody
    @ApiOperation(value="非计划转入ICU率",httpMethod="POST",notes="非计划转入ICU率")
    public String searchNonPlanToICURateSYZXYY(@ApiParam(name="searchConditionFormBean", value ="统计查询参数") @RequestBody SearchConditionFormBean searchConditionFormBean) {
        logger.info("begin searchNonPlanToICURate");
        ResponseValue resp = new ResponseValue();
        SearchFormBean searchFormBean = new SearchFormBean();
        searchFormBean.setStartTime(searchConditionFormBean.getStartTime());
        ...
        //非计划转入ICU
        List<SearchOperFormBean> toICUQmFjhOperList = qcManagerService.searchAnaesQualityControlByType("5", searchFormBean);
        int toICUOperTotal = 0;
     ...
     * @see [类、类#方法、类#成员]
     */
    @RequestMapping(value = "/searchAnesSecondIntubatRateSYZXYY")
    @ResponseBody
    @ApiOperation(value="非计划二次气管插管率",httpMethod="POST",notes="非计划二次气管插管率")
    public String searchAnesSecondIntubatRateSYZXYY(@ApiParam(name="searchConditionFormBean", value ="查询参数") @RequestBody SearchConditionFormBean searchConditionFormBean) {
        logger.info("begin searchAnesSecondIntubatRate");
        ResponseValue resp = new ResponseValue();
        /**
         * 非计划二次气管插管患者数
        ...
        List<SearchOperFormBean> secondIntubatList = qcManagerService.searchAnaesQualityControlByType("6", searchFormBean);
        /**
         *同期术后气管插管拔除患者总数
         */
        List<SearchOperFormBean> extubatList = qcManagerService.searchAnesExtubatList(searchFormBean);
     ...
     * @see [类、类#方法、类#成员]
     */
    @RequestMapping(value = "/searchAutograftBloodTrans400mlInfoSYZXYY")
    @ResponseBody
    @ApiOperation(value="术中自体血输注率",httpMethod="POST",notes="术中自体血输注率")
    public String searchAutograftBloodTrans400mlInfoSYZXYY(@ApiParam(name="searchConditionFormBean", value ="统计查询参数") @RequestBody SearchConditionFormBean searchConditionFormBean)
    {
        logger.info("begin searchAutograftBloodTrans400mlInfo");
        ResponseValue resp = new ResponseValue();
        SearchFormBean searchFormBean = new SearchFormBean();
        ...
        //自体血
        List<SearchOperFormBean> autograftList = qcManagerService.searchAnaesQualityControlByType("7", searchFormBean);
        //输血
        List<SearchOperFormBean> operList = qcManagerService.searchAnaesQualityControlByType("8", searchFormBean);
     ...
     * @see [类、类#方法、类#成员]
     */
    @RequestMapping(value = "/search24HourDealthRateSYZXYY")
    @ResponseBody
    @ApiOperation(value="查询麻醉开始后24小时内死亡率",httpMethod="POST",notes="查询麻醉开始后24小时内死亡率")
    public String search24HourDealthRateSYZXYY(@ApiParam(name="searchConditionFormBean", value ="查询参数") @RequestBody SearchConditionFormBean searchConditionFormBean) {
        logger.info("begin search24HourDealthRate");
        ResponseValue resp = new ResponseValue();
        SearchFormBean searchFormBean = new SearchFormBean();
        searchFormBean.setStartTime(searchConditionFormBean.getStartTime());
        ...
        int operTotal = 0;
        List<SearchOperFormBean> operList = qcManagerService.searchOperByCondition(searchFormBean);
        List<SearchOperFormBean> hourDeathList = qcManagerService.searchAnaesQualityControlByType("9", searchFormBean);
     ...
     * @see [类、类#方法、类#成员]
     */
    @RequestMapping(value = "/searchAnes24hrCardiacArrestRateSYZXYY")
    @ResponseBody
    @ApiOperation(value="麻醉开始后24小时内心跳骤停率",httpMethod="POST",notes="麻醉开始后24小时内心跳骤停率")
    public String searchAnes24hrCardiacArrestRateSYZXYY(@ApiParam(name="searchConditionFormBean", value ="查询参数") @RequestBody SearchConditionFormBean searchConditionFormBean) {
        logger.info("begin searchAnes24hrCardiacArrestRate");
        ResponseValue resp = new ResponseValue();
        SearchFormBean searchFormBean = new SearchFormBean();
        searchFormBean.setStartTime(searchConditionFormBean.getStartTime());
        ...
        int operTotal = 0;
        List<SearchOperFormBean> operList = qcManagerService.searchOperByCondition(searchFormBean);
        List<SearchOperFormBean> hrCardiacArrestList = qcManagerService.searchAnaesQualityControlByType("10", searchFormBean);
     ...
     * @see [类、类#方法、类#成员]
     */
    @RequestMapping(value = "/searchAnesAllergicRecatRateSYZXYY")
    @ResponseBody
    @ApiOperation(value="麻醉期间严重过敏反应发生率",httpMethod="POST",notes="麻醉期间严重过敏反应发生率")
    public String searchAnesAllergicRecatRateSYZXYY(@ApiParam(name="searchConditionFormBean", value ="查询参数") @RequestBody SearchConditionFormBean searchConditionFormBean) {
        logger.info("begin searchAnesAllergicRecatRate");
        ResponseValue resp = new ResponseValue();
        SearchFormBean searchFormBean = new SearchFormBean();
        searchFormBean.setStartTime(searchConditionFormBean.getStartTime());
        ...
        //在麻醉记录单二中有循环衰竭 circulatFailure 、严重气道反应 airwayResp、明显皮疹 obviousRash、肾上腺素 adrenaline
        List<SearchOperFormBean> allergicReactionList = qcManagerService.searchAnaesQualityControlByType("11", searchFormBean);
        /**
         * 同期麻醉总例数
         */
        List<SearchOperFormBean> anaesList = qcManagerService.searchOperByCondition(searchFormBean);
     ...
     * @see [类、类#方法、类#成员]
     */
    @RequestMapping(value = "/searchVenipunctyreRateSYZXYY")
    @ResponseBody
    @ApiOperation(value="中心静脉穿刺严重并发症发生率",httpMethod="POST",notes="中心静脉穿刺严重并发症发生率")
    public String searchVenipunctyreRateSYZXYY(@ApiParam(name="searchConditionFormBean", value ="查询参数") @RequestBody SearchConditionFormBean searchConditionFormBean) {
        logger.info("begin searchVenipunctyreRate");
        ResponseValue resp = new ResponseValue();
        SearchFormBean searchFormBean = new SearchFormBean();
        searchFormBean.setStartTime(searchConditionFormBean.getStartTime());
        ...
        List<SearchOperFormBean> venipunctyreList = qcManagerService.searchAnaesQualityControlByType("12", searchFormBean);
        Integer venListTotal = venipunctyreList.size();
        resp.put("venipunctyreList", venipunctyreList);
        resp.put("venListTotal", venListTotal);
        
        List<SearchOperFormBean> operList = qcManagerService.searchAnaesQualityControlByType("13", searchFormBean);
        Integer operTotal = operList.size();
        resp.put("operList", operList);
        resp.put("operTotal", operTotal);
     ...
     * @see [类、类#方法、类#成员]
     */
    @RequestMapping(value = "/searchSpinalAnaesNeuralComplicatRateSYZXYY")
    @ResponseBody
    @ApiOperation(value="椎管内麻醉后严重神经并发症发生率",httpMethod="POST",notes="椎管内麻醉后严重神经并发症发生率")
    public String searchSpinalAnaesNeuralComplicatRateSYZXYY(@ApiParam(name="searchConditionFormBean", value ="查询参数") @RequestBody SearchConditionFormBean searchConditionFormBean) {
        logger.info("begin searchSpinalAnaesNeuralComplicatRate");
        ResponseValue resp = new ResponseValue();
        SearchFormBean searchFormBean = new SearchFormBean();
        searchFormBean.setStartTime(searchConditionFormBean.getStartTime());
        ...
        List<SearchOperFormBean> spinalList = qcManagerService.searchAnaesQualityControlByType("14", searchFormBean);//椎管内全麻
        
        List<SearchOperFormBean> spinalComplicatList = qcManagerService.searchAnaesQualityControlByType("15", searchFormBean);
     ...
     * @see [类、类#方法、类#成员]
     */
    @RequestMapping(value = "/searchGenAnesHoarseRateSYZXYY")
    @ResponseBody
    @ApiOperation(value="全麻气管插管拔管后声音嘶哑发生率",httpMethod="POST",notes="全麻气管插管拔管后声音嘶哑发生率")
    public String searchGenAnesHoarseRateSYZXYY(@ApiParam(name="searchConditionFormBean", value ="查询参数") @RequestBody SearchConditionFormBean searchConditionFormBean) {
        logger.info("begin searchGenAnesHoarseRate");
        ResponseValue resp = new ResponseValue();
        SearchFormBean searchFormBean = new SearchFormBean();
        searchFormBean.setStartTime(searchConditionFormBean.getStartTime());
        ...
        List<SearchOperFormBean> extubatList = qcManagerService.searchAnaesQualityControlByType("17", searchFormBean);
        /**
         * 同期全麻气管插管总例数
         */
        List<SearchOperFormBean> tubesList = qcManagerService.searchAnaesQualityControlByType("16", searchFormBean);
     ...
     * @see [类、类#方法、类#成员]
     */
    @RequestMapping(value = "/searchAnesComaRateSYZXYY")
    @ResponseBody
    @ApiOperation(value="麻醉后新发昏迷发生率",httpMethod="POST",notes="麻醉后新发昏迷发生率")
    public String searchAnesComaRateSYZXYY(@ApiParam(name="searchConditionFormBean", value ="查询参数") @RequestBody SearchConditionFormBean searchConditionFormBean) {
        logger.info("begin searchAnesComaRate");
        ResponseValue resp = new ResponseValue();
        SearchFormBean searchFormBean = new SearchFormBean();
        searchFormBean.setStartTime(searchConditionFormBean.getStartTime());
        ...
        //持续昏迷超过24小时
        List<SearchOperFormBean> comaList = qcManagerService.searchAnaesQualityControlByType("18", searchFormBean);
    ...
    /*@RequestMapping(value = "/searchAnaesQualityControl")
    @ResponseBody
    @ApiOperation(value="查询麻醉质控",httpMethod="POST",notes="查询麻醉质控")
    public String searchAnaesQualityControl(@ApiParam(name="searchConditionFormBean", value ="查询参数") @RequestBody SearchConditionFormBean searchConditionFormBean)
    {
        logger.info("begin searchAnaesQualityControl");
        ResponseValue resp = new ResponseValue();
        int molecular = 0;   //分子
        int denominator = 0; //分母
        List molecularList = new ArrayList();
        List denominatorList = new ArrayList();
        
        String qcType = searchConditionFormBean.getQcType();
        String beid = getBeid();
        ...
        searchFormBean.setBeid(beid);
        
        if ("1".equals(qcType))
        {
            String recordMonth = startTime.substring(0, 7);
            BasAnaesDoctorAmount anaesDoctorAmount = basAnaesDoctorAmountService.selectByPrimaryKey(recordMonth, beid);
            molecular = (null == anaesDoctorAmount || null == anaesDoctorAmount.getAmount()) ? 0 : anaesDoctorAmount.getAmount();
            denominatorList = qcManagerService.searchOperByCondition(searchFormBean);
            denominator = denominatorList.size();
        }
        else if ("2".equals(qcType))
        {
            denominatorList = qcManagerService.searchOperByCondition(searchFormBean);
            denominator = denominatorList.size();
        ...
        resp.put("molecular", molecular);
        resp.put("denominator", denominator);
        resp.put("molecularList", molecularList);
        resp.put("denominatorList", denominatorList);
        
        float rate = 0.00f;
        if (molecular != 0 && denominator != 0)
        {
            rate = (float)molecular / denominator;
        }
        DecimalFormat df = new DecimalFormat("0.000");
        BigDecimal b = new BigDecimal(df.format(rate));
        BigDecimal c = new BigDecimal(Double.valueOf(100));
        resp.put("rate", b.multiply(c).setScale(1, BigDecimal.ROUND_HALF_UP).floatValue());
        logger.info("end searchAnaesQualityControl");
        return resp.getJsonStr();
    }*/
}
