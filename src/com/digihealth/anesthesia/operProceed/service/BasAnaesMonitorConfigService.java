...
public class BasAnaesMonitorConfigService extends BaseService {
	...
	public List<BasAnaesMonitorConfigFormBean> getAnaesRecordShowListByRegOptId(BaseInfoQuery baseQuery) {
	    ...
	    if (StringUtils.isBlank(baseQuery.getOperRoomId()))
	    {
	        baseQuery.setOperRoomId(getRoomIdByDocType(baseQuery.getRegOptId(), baseQuery.getDocType()));
	    }
	...
	public List<BasAnaesMonitorConfigFormBean> getAnaesMonitorRecordShowListByRegOptId(BaseInfoQuery baseQuery) {
		...
		if (null != baseQuery && StringUtils.isBlank(baseQuery.getOperRoomId()))
        {
            baseQuery.setOperRoomId(getRoomIdByDocType(baseQuery.getRegOptId(), baseQuery.getDocType()));
        }
				...
				if (monitorConfig.getEventName().equals(anaesMonitorConfigFormBean.getEventName())) {
					...
					if (!eventStr.contains(anaesMonitorConfigFormBean.getEventName())) {
						...
						if (null != amcf) {
							...
							anaesMonitorConfigFormBean.setWidthAndHeight(amcf.getWidthAndHeight());
							anaesMonitorConfigFormBean.setColor(amcf.getColor());
						}
						//采集项未选中时，在b_anaes_monitor_config表中不存在记录，取默认配置表中的默认eventId
                        else
                        {
                            BasMonitorConfigDefault monitorConfigDefault = basMonitorConfigDefaultDao.selectByEventName(anaesMonitorConfigFormBean.getEventName(), getBeid(), baseQuery.getDocType());
	 ...
	 */
	@Transactional
	public void saveAnaesMonitorConfig(UpdAnaesMonitorConfigFormbean formbean) {

	    List<BasAnaesMonitorConfig> anaesMonitorConfigList = formbean.getCheckList();
        String regOptId = formbean.getRegOptId();
        Integer docType = formbean.getDocType();
        BasAnaesMonitorConfig queryObj = new BasAnaesMonitorConfig();
        queryObj.setRegOptId(regOptId);
        queryObj.setPosition(0);
        queryObj.setDocType(docType);
		...
		if (anaesMonitorConfigList.size() > 0) {
			for (BasAnaesMonitorConfig anaesMonitorConfig : anaesMonitorConfigList) { 
				...
				BasMonitorConfigDefault monitorConfigDefault = basMonitorConfigDefaultDao.selectByEventName(anaesMonitorConfig.getEventName(),beid, docType);
     ...
     */
    @Transactional
    public void updAnaesMonitorConfig(UpdAnaesMonitorConfigFormbean formbean) {
	...
	public List<BasAnaesMonitorConfigFormBean> finaAnaesListWithoutO2(BaseInfoQuery baseQuery) {
		...
		List<BasAnaesMonitorConfigFormBean> anaesMonitorList = basAnaesMonitorConfigDao.getAnaesMonitorConfigListByRegOptId(baseQuery);
		for (BasAnaesMonitorConfigFormBean anaesMonitorConfigFormBean : anaesMonitorList) {
			if (MyConstants.O2_EVENT_ID.equals(anaesMonitorConfigFormBean.getEventId())) {
